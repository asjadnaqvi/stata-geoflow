...
{title:geoflow}: A Stata module for generating flow arcs.

The package assumes that the users have the following two files:
- {bf:file1}: This file should contain a numeric {bf:numvar} flow variable identified by {opt from()} and {opt to()}
that contain the identifiers. The file should already be loaded before using the command.
- {bf:file2}: This file should contain a unique {opt key()} variable that defines the coordinates of the {opt from()} and {opt to()} variables.
